...
[TestClass]
public sealed class InstructionSTOSWTests : InstructionConstants<InstructionSTOSW>
{
    [TestMethod]
    ...
    {
        List<IPrefix> validPrefixes = [new PrefixREP()];
    ...
    public void TestExecuteMethod_NoOperands()
    {
        ...
        virtualMachine.CPU.GetRegister<VirtualRegisterRAX>().AX = 0x_af_ed;
        ...
        Assert.AreEqual<ushort>(0x_af_ed, virtualMachine.GetWord(0));
        Assert.AreNotEqual<ulong>(0, virtualMachine.CPU.GetRegister<VirtualRegisterRIP>().RIP);
    }

    [TestMethod]
    public void TestExecuteMethod_MemoryOperand()
    {
        IInstruction instruction = GenerateInstruction(InstructionVariant.OneOperandMemory(), null, GenerateOperand("test_label", OperandVariant.Memory, Size.Byte));
        IVirtualMachine virtualMachine = GenerateVirtualMachine();
