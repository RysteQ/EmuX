...
[TestClass]
public sealed class InstructionLEATests : InstructionConstants<InstructionLEA>
{
    [TestMethod]
    ...
    public void TestIsValidMethod_AllPrefixes_ValidVariant_CorrectPrefixCheck()
    {
        ...
        {
            IInstruction instruction = GenerateInstruction(InstructionVariant.TwoOperandsRegisterMemory(), prefix, GenerateOperand("ax", OperandVariant.Register, Size.Word), GenerateOperand("[test_label]", OperandVariant.Memory, Size.Word, [GenerateMemoryOffset(MemoryOffsetType.Label, MemoryOffsetOperand.NaN, "test_label")]));
    ...
    [TestMethod]
    public void TestExecuteMethod_LoadEffectiveAddress_EAX()
    {
        IInstruction instruction = GenerateInstruction(InstructionVariant.TwoOperandsRegisterMemory(), GeneratePrefix(), GenerateOperand("ecx", OperandVariant.Register, Size.Word), GenerateOperand("[eax * 4]", OperandVariant.Memory, Size.Word, [GenerateMemoryOffset(MemoryOffsetType.Register, MemoryOffsetOperand.Addition, "eax"), GenerateMemoryOffset(MemoryOffsetType.Integer, MemoryOffsetOperand.Multiplication, "4")]));
        IVirtualMachine virtualMachine = GenerateVirtualMachine();

        virtualMachine.CPU.GetRegister<VirtualRegisterRAX>().EAX = 0;
        instruction.Execute(virtualMachine);

        Assert.AreEqual<uint>(0, virtualMachine.CPU.GetRegister<VirtualRegisterRCX>().ECX);
