...
[TestClass]
public sealed class InstructionPUSHFTests : InstructionConstants<InstructionPUSHF>
{
    [TestMethod]
    ...
    [TestMethod]
    public void TestExecuteMethod_PushZeroedWordToStack()
    {
    ...
    [TestMethod]
    public void TestExecuteMethod_PushRandomWordToStack()
    {
        IInstruction instruction = GenerateInstruction(InstructionVariant.TwoOperandsRegisterValue());
        IVirtualMachine virtualMachine = GenerateVirtualMachine();
        ushort valueToPushAndPop = (ushort)Random.Shared.NextInt64();

        virtualMachine.CPU.GetRegister<VirtualRegisterEFLAGS>().FLAGS = valueToPushAndPop;
        instruction.Execute(virtualMachine);
