...
[TestClass]
public sealed class InstructionPOPFQTests : InstructionConstants<InstructionPOPFQ>
{
    [TestMethod]
    ...
    [TestMethod]
    public void TestExecuteMethod_PopZeroedQuadToFlags()
    {
        ...
        Assert.AreEqual<ulong>(0, virtualMachine.CPU.GetRegister<VirtualRegisterEFLAGS>().RFLAGS);
    }

    [TestMethod]
    public void TestExecuteMethod_PopRandomQuadToFlags()
    {
        IInstruction instruction = GenerateInstruction(InstructionVariant.TwoOperandsRegisterValue(), GeneratePrefix());
        IVirtualMachine virtualMachine = GenerateVirtualMachine();
        ...
        Assert.AreEqual<ulong>(valueToPushAndPop, virtualMachine.CPU.GetRegister<VirtualRegisterEFLAGS>().RFLAGS);
    }
}