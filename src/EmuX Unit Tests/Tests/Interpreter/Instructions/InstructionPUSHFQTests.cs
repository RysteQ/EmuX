...
[TestClass]
public sealed class InstructionPUSHFQTests : InstructionConstants<InstructionPUSHFQ>
{
    [TestMethod]
    ...
    [TestMethod]
    public void TestExecuteMethod_PushZeroedQuadToStack()
    {
        IInstruction instruction = GenerateInstruction(InstructionVariant.TwoOperandsRegisterValue());
        IVirtualMachine virtualMachine = GenerateVirtualMachine();

        virtualMachine.CPU.GetRegister<VirtualRegisterEFLAGS>().RFLAGS = 0;
    ...
    [TestMethod]
    public void TestExecuteMethod_PushRandomQuadToStack()
    {
        IInstruction instruction = GenerateInstruction(InstructionVariant.TwoOperandsRegisterValue());
        IVirtualMachine virtualMachine = GenerateVirtualMachine();
        ulong valueToPushAndPop = (ulong)Random.Shared.NextInt64();

        virtualMachine.CPU.GetRegister<VirtualRegisterEFLAGS>().RFLAGS = valueToPushAndPop;
        instruction.Execute(virtualMachine);
