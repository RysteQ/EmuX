...
[TestClass]
public sealed class InstructionLDSTests : InstructionConstants<InstructionLDS>
{
    [TestMethod]
    ...
    [TestMethod]
    public void TestExecuteMethod_Transfer_20h_30h_AX_DS()
    {
        IInstruction instruction = GenerateInstruction(InstructionVariant.TwoOperandsRegisterMemory(), GenerateOperand("ax", OperandVariant.Register, Size.Word), GenerateOperand("[test_label]", OperandVariant.Memory, Size.Word, [GenerateMemoryOffset(MemoryOffsetType.Label, MemoryOffsetOperand.NaN, "test_label")])); // TODO
        ...
        Assert.AreEqual<ushort>(0x_30, virtualMachine.CPU.GetRegister<VirtualRegisterDS>().DS);
    }

    [TestMethod]
    public void TestExecuteMethod_Transfer_40h_10h_AX_DS()
    {
        IInstruction instruction = GenerateInstruction(InstructionVariant.TwoOperandsRegisterMemory(), GenerateOperand("ax", OperandVariant.Register, Size.Word), GenerateOperand("[test_label]", OperandVariant.Memory, Size.Word, [GenerateMemoryOffset(MemoryOffsetType.Label, MemoryOffsetOperand.NaN, "test_label")])); // TODO
        IVirtualMachine virtualMachine = GenerateVirtualMachine();
        ...
        Assert.AreEqual<ushort>(0x_10, virtualMachine.CPU.GetRegister<VirtualRegisterDS>().DS);
    }
}