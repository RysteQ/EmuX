...
[TestClass]
public sealed class InstructionPOPFDTests : InstructionConstants<InstructionPOPFD>
{
    [TestMethod]
    ...
    {
        IInstruction instruction = GenerateInstruction(InstructionVariant.TwoOperandsMemoryRegister(), GenerateOperand("10", OperandVariant.Value, Size.Byte), GenerateOperand("al", OperandVariant.Register, Size.Byte));
    ...
    [TestMethod]
    public void TestExecuteMethod_PopZeroedDoubleToFlags()
    {
        ...
        Assert.AreEqual<uint>(0, virtualMachine.CPU.GetRegister<VirtualRegisterEFLAGS>().EFLAGS);
    }

    [TestMethod]
    public void TestExecuteMethod_PopRandomDoubleToFlags()
    {
        IInstruction instruction = GenerateInstruction(InstructionVariant.TwoOperandsRegisterValue());
        IVirtualMachine virtualMachine = GenerateVirtualMachine();
        ...
        Assert.AreEqual<uint>(valueToPushAndPop, virtualMachine.CPU.GetRegister<VirtualRegisterEFLAGS>().EFLAGS);
    }
}