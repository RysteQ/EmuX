...
[TestClass]
public sealed class InstructionCDQTests : InstructionConstants<InstructionCDQ>
{
    [TestMethod]
    ...
    public void TestExecuteMethod_DoExtendSign()
    {
        ...
        Assert.AreEqual<uint>(uint.MaxValue, virtualMachine.CPU.GetRegister<VirtualRegisterRDX>().EDX);
    }
    ...
    public void TestExecuteMethod_DoNotExtendSign()
    {
        ...
        virtualMachine.CPU.GetRegister<VirtualRegisterRDX>().EDX = uint.MaxValue;
        instruction.Execute(virtualMachine);

        Assert.AreEqual<uint>(0, virtualMachine.CPU.GetRegister<VirtualRegisterRDX>().EDX);
    }
}