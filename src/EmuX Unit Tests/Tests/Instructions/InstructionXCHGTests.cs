...
[TestClass]
public sealed class InstructionXCHGTests : InstructionConstants<InstructionXCHG>
{
    [TestMethod]
    ...
    [TestMethod]
    public void TestExecuteMethod_SwapRegisterRegister()
    {
        IInstruction instruction = GenerateInstruction(InstructionVariant.TwoOperandsRegisterRegister(), GeneratePrefix(), GenerateOperand("AL", OperandVariant.Register, Size.Byte), GenerateOperand("CL", OperandVariant.Register, Size.Byte));
        IVirtualMachine virtualMachine = GenerateVirtualMachine();

        virtualMachine.CPU.GetRegister<VirtualRegisterRAX>().AL = 64;
        virtualMachine.CPU.GetRegister<VirtualRegisterRCX>().CL = 100;
        instruction.Execute(virtualMachine);

        Assert.AreEqual<byte>(100, virtualMachine.CPU.GetRegister<VirtualRegisterRAX>().AL);
        Assert.AreEqual<byte>(64, virtualMachine.CPU.GetRegister<VirtualRegisterRCX>().CL);
    }

    [TestMethod]
    public void TestExecuteMethod_SwapRegisterMemory()
    {
        ...
        Assert.AreEqual<byte>(100, virtualMachine.CPU.GetRegister<VirtualRegisterRAX>().AL);
        Assert.AreEqual<byte>(64, virtualMachine.GetByte(0));
    }

    [TestMethod]
    public void TestExecuteMethod_SwapMemoryRegister()
    {
