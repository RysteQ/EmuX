...
public sealed class InstructionINTTests : InstructionConstants<InstructionINT>
{
    ...
    public void TestExecuteMethod_RTCInterrupt_SetAndReadSystemClock()
    {
        ...
        Assert.AreEqual((byte)currentTime.Hour, virtualMachine.CPU.GetRegister<VirtualRegisterRCX>().CH);
        Assert.AreEqual((byte)currentTime.Minute, virtualMachine.CPU.GetRegister<VirtualRegisterRCX>().CL);
        Assert.AreEqual((byte)currentTime.Second, virtualMachine.CPU.GetRegister<VirtualRegisterRDX>().DH);
    }
