...
public sealed class InstructionADCTests : InstructionConstants<InstructionADC>
{
    ...
    {
        IInstruction instruction = GenerateInstruction(InstructionVariant.TwoOperandsMemoryValue(), GeneratePrefix(), GenerateOperand("[test_label]", OperandVariant.Memory, Size.Qword, [GenerateMemoryOffset(MemoryOffsetType.Label, MemoryOffsetOperand.NaN, "test_label")]), GenerateOperand("10h", OperandVariant.Value, Size.Dword));
        IVirtualMachine virtualMachine = GenerateVirtualMachine();
