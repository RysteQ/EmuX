...
[TestClass]
public sealed class InstructionPOPFTests : InstructionConstants<InstructionPOPF>
{
    [TestMethod]
    ...
    [TestMethod]
    public void TestExecuteMethod_PopZeroedWordToFlags()
    {
        ...
        Assert.AreEqual<ushort>(0, virtualMachine.CPU.GetRegister<VirtualRegisterEFLAGS>().FLAGS);
        Assert.AreNotEqual<ulong>(0, virtualMachine.CPU.GetRegister<VirtualRegisterRIP>().RIP);
    }

    [TestMethod]
    public void TestExecuteMethod_PopRandomWordToFlags()
    {
        IInstruction instruction = GenerateInstruction(InstructionVariant.TwoOperandsRegisterValue(), GeneratePrefix());
        IVirtualMachine virtualMachine = GenerateVirtualMachine();
        ...
        Assert.AreEqual<ushort>(valueToPushAndPop, virtualMachine.CPU.GetRegister<VirtualRegisterEFLAGS>().FLAGS);
        Assert.AreNotEqual<ulong>(0, virtualMachine.CPU.GetRegister<VirtualRegisterRIP>().RIP);
    }
}