...
public sealed class InstructionROL : IInstruction
{
    public InstructionROL(InstructionVariant variant, IPrefix? prefix, IOperand? firstOperand, IOperand? secondOperand, IOperand? thirdOperand, IOperandDecoder operandDecoder, IFlagStateProcessor flagStateProcessor, IInstructionEncoder instructionEncoder)
    {
        Variant = variant;
        Prefix = prefix;
    ...
    public void Execute(IVirtualMachine virtualMachine)
    {
        ...
        for (int i = 0; i < (int)bitsToShift; i++)
        {
            ...
            if ((valueToShift >> (int)FirstOperand!.OperandSize * 8 - (i + 1)) % 2 == 1)
            {
                valueToSet++;
            }
