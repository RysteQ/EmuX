...
public sealed class InstructionPUSHFQ : IInstruction
{
    public InstructionPUSHFQ(InstructionVariant variant, IPrefix? prefix, IOperand? firstOperand, IOperand? secondOperand, IOperand? thirdOperand, IOperandDecoder operandDecoder, IFlagStateProcessor flagStateProcessor, IInstructionEncoder instructionEncoder)
    {
        Variant = variant;
        Prefix = prefix;
    ...
    {
        virtualMachine.PushQuad(virtualMachine.CPU.GetRegister<VirtualRegisterEFLAGS>().RFLAGS);
