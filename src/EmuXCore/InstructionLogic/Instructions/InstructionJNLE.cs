...
public sealed class InstructionJNLE : IInstruction
{
    public InstructionJNLE(InstructionVariant variant, IPrefix? prefix, IOperand? firstOperand, IOperand? secondOperand, IOperand? thirdOperand, IOperandDecoder operandDecoder, IFlagStateProcessor flagStateProcessor, IInstructionEncoder instructionEncoder)
    {
        Variant = variant;
        Prefix = prefix;
    ...
    {
        if (!virtualMachine.GetFlag(EFlags.ZF) && virtualMachine.GetFlag(EFlags.SF) == virtualMachine.GetFlag(EFlags.OF))
        {
