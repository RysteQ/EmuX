...
public sealed class InstructionJNBE : IInstruction
{
    public InstructionJNBE(InstructionVariant variant, IPrefix? prefix, IOperand? firstOperand, IOperand? secondOperand, IOperand? thirdOperand, IOperandDecoder operandDecoder, IFlagStateProcessor flagStateProcessor, IInstructionEncoder instructionEncoder)
    {
        Variant = variant;
        Prefix = prefix;
    ...
    {
        if (!virtualMachine.GetFlag(EFlags.CF) && !virtualMachine.GetFlag(EFlags.ZF))
        {
