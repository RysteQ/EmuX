...
public sealed class InstructionPOPF : IInstruction
{
    public InstructionPOPF(InstructionVariant variant, IPrefix? prefix, IOperand? firstOperand, IOperand? secondOperand, IOperand? thirdOperand, IOperandDecoder operandDecoder, IFlagStateProcessor flagStateProcessor, IInstructionEncoder instructionEncoder)
    {
        Variant = variant;
        Prefix = prefix;
    ...
    {
        virtualMachine.CPU.GetRegister<VirtualRegisterEFLAGS>().FLAGS = virtualMachine.PopWord();
