...
public sealed class InstructionJNS : IInstruction
{
    public InstructionJNS(InstructionVariant variant, IPrefix? prefix, IOperand? firstOperand, IOperand? secondOperand, IOperand? thirdOperand, IOperandDecoder operandDecoder, IFlagStateProcessor flagStateProcessor, IInstructionEncoder instructionEncoder)
    {
        Variant = variant;
        Prefix = prefix;
    ...
    {
        if (!virtualMachine.GetFlag(EFlags.SF))
        {
