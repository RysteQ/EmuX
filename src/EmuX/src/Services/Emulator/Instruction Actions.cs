...
public class InstructionActions
{
    ...
    {
        int low_nibble_al = virtual_system.GetRegisterByte(Registers.RAX, false) & 0x0F;
        ...
        {
            virtual_system.SetRegisterByte(Registers.RAX, (byte)(low_nibble_al - 6), false);
            virtual_system.SetRegisterByte(Registers.RAX, (byte)(virtual_system.GetRegisterByte(Registers.RAX, true) - 1), true);
    ...
    public ulong DEC(ulong value, Size bit_mode)
    {
        switch (bit_mode)
        {
            case Size._8_BIT:
    ...
    public VirtualSystem DIV(VirtualSystem virtual_system, Size bit_mode, ulong destination_value)
    {
        ulong quotient = 0;
        ulong remainder = 0;
        ulong divident = 0;
        ...
        {
            case Size._8_BIT:
                divident = virtual_system.GetRegisterQuad(Registers.RAX);
                quotient = divident / destination_value;
                remainder = divident % destination_value;

                virtual_system.SetRegisterByte(Registers.RAX, (byte)quotient, false);
                virtual_system.SetRegisterByte(Registers.RAX, (byte)remainder, true);

                break;

            case Size._16_BIT:
                divident = (ulong)((virtual_system.GetRegisterWord(Registers.RDX) << 16) + virtual_system.GetRegisterWord(Registers.RAX));

                quotient = divident / destination_value;
                remainder = divident % destination_value;

                virtual_system.SetRegisterWord(Registers.RAX, (ushort)quotient);
                virtual_system.SetRegisterWord(Registers.RDX, (ushort)remainder);

                break;

            case Size._32_BIT:
                divident = (virtual_system.GetRegisterDouble(Registers.RDX) << 32) + virtual_system.GetRegisterDouble(Registers.RAX);

                quotient = divident / destination_value;
                remainder = divident % destination_value;

                virtual_system.SetRegisterDouble(Registers.RAX, (uint)quotient);
                virtual_system.SetRegisterDouble(Registers.RDX, (uint)remainder);
    ...
    public VirtualSystem MUL(VirtualSystem virtual_system, ulong destination_value, Size bit_mode)
    {
        if (bit_mode == Size._8_BIT)
        {
            virtual_system.SetRegisterWord(Registers.RAX, (ushort)(virtual_system.GetRegisterByte(Registers.RAX, false) * destination_value));
        }
        else if (bit_mode == Size._16_BIT)
        {
            virtual_system.SetRegisterWord(Registers.RDX, (ushort)(virtual_system.GetRegisterWord(Registers.RAX) * destination_value & 0xFFFF0000));
            virtual_system.SetRegisterWord(Registers.RAX, (ushort)(virtual_system.GetRegisterWord(Registers.RAX) * destination_value & 0x0000FFFF));
        }
        else
        {
            virtual_system.SetRegisterDouble(Registers.RDX, (uint)(virtual_system.GetRegisterDouble(Registers.RAX) * destination_value & 0xFFFFFFFF00000000));
            virtual_system.SetRegisterDouble(Registers.RAX, (uint)(virtual_system.GetRegisterDouble(Registers.RAX) * destination_value & 0x00000000FFFFFFFF));
        }
    ...
    public VirtualSystem POP(VirtualSystem virtual_system, Instruction instruction, int memory_index)
    {
        ...
        {
            case Size._8_BIT: value_to_set = virtual_system.PopByte(); break;
            case Size._16_BIT: value_to_set = virtual_system.PopWord(); break;
            case Size._32_BIT: value_to_set = virtual_system.PopDouble(); break;
            case Size._64_BIT: value_to_set = virtual_system.PopQuad(); break;
        }

        // the set value function taken from Emulator.cs
        // check if the value needs to be saved in a register or memory location
        if (instruction.destination_register != Registers.NoN && instruction.destination_pointer == false)
        {
            switch (instruction.bit_mode)
            {
                case Size._8_BIT: virtual_system.SetRegisterByte(instruction.destination_register, (byte)value_to_set, instruction.destination_high_or_low); break;
                case Size._16_BIT: virtual_system.SetRegisterWord(instruction.destination_register, (ushort)value_to_set); break;
                case Size._32_BIT: virtual_system.SetRegisterDouble(instruction.destination_register, (uint)value_to_set); break;
                case Size._64_BIT: virtual_system.SetRegisterQuad(instruction.destination_register, value_to_set); break;
            }
        }
        else
        {
            switch (instruction.bit_mode)
            {
                case Size._8_BIT: virtual_system.SetByteMemory(memory_index, (byte)value_to_set); break;
                case Size._16_BIT: virtual_system.SetWordMemory(memory_index, (ushort)value_to_set); break;
                case Size._32_BIT: virtual_system.SetDoubleMemory(memory_index, (uint)value_to_set); break;
                case Size._64_BIT: virtual_system.SetQuadMemory(memory_index, value_to_set); break;
            }
        }
    ...
    public VirtualSystem PUSH(VirtualSystem virtual_system, Size bit_mode, ulong value)
    {
        switch (bit_mode)
        {
            case Size._8_BIT: virtual_system.PushByte((byte)value); break;
            case Size._16_BIT: virtual_system.PushWord((ushort)value); break;
            case Size._32_BIT: virtual_system.PushDouble((uint)value); break;
            case Size._64_BIT: virtual_system.PushQuad(value); break;
        }
    ...
    public VirtualSystem RCL(VirtualSystem virtual_system, Instruction instruction, int memory_index, ulong destination_value, int bits_to_shift)
    {
        ...
        for (int i = 0; i < bits_to_shift; i++)
        {
            ...
            if (CF_new_value)
                virtual_system.EFLAGS ^= virtual_system.GetEFLAGSMasks()[0] + 1;
            else
                virtual_system.EFLAGS ^= virtual_system.GetEFLAGSMasks()[0];
        }

        switch (instruction.bit_mode)
        {
            case Size._8_BIT: value_to_set = new_value_byte; break;
            case Size._16_BIT: value_to_set = new_value_ushort; break;
            case Size._32_BIT: value_to_set = new_value_uint; break;
            case Size._64_BIT: value_to_set = new_value_ulong; break;
        }
    ...
    public ulong ROL(Size bit_mode, ulong value, int bits_to_shift)
    {
        ...
        switch (bit_mode)
        {
            case Size._8_BIT: return (ulong)(new_value_byte << bits_to_shift | new_value_byte >> 8 - bits_to_shift);
            case Size._16_BIT: return (ulong)(new_value_ushort << bits_to_shift | new_value_ushort >> 16 - bits_to_shift);
            case Size._32_BIT: return new_value_uint << bits_to_shift | new_value_uint << 32 - bits_to_shift;
            case Size._64_BIT: return new_value_ulong << bits_to_shift | new_value_ulong >> 64 - bits_to_shift;
            default: return 0;
        }
    }

    public ulong ROR(Size bit_mode, ulong value, int bits_to_shift)
    {
        byte new_value_byte = (byte)value;
        ushort new_value_ushort = (ushort)value;
        uint new_value_uint = (uint)value;
